...
void LSystem::processPy(unsigned int n,
    ...
    flowers.reserve(3 * preFlowers.size());
    for (const Geometry& flower : preFlowers) {
        if (flower.second == "*") {
            flowers.emplace_back(std::vector<float> {
                static_cast<float>(flower.first[0]),
                    static_cast<float>(flower.first[1]),
                    static_cast<float>(flower.first[2])
            });
        }
    }

    branches.reserve(6 * preBranches.size());
    for (const Branch& branch : preBranches) {
        branches.emplace_back(std::vector<float> {
            static_cast<float>(branch.first[0]),
                static_cast<float>(branch.first[1]),
                static_cast<float>(branch.first[2]),
                static_cast<float>(branch.second[0]),
                static_cast<float>(branch.second[1]),
                static_cast<float>(branch.second[2]),
        });
    }
